...
std::vector<double> get_features (vpFeaturePoint p[4])
{
  ...
  // omega_x
  double a = fabs (vec1X);
  double b = fabs (p[3].get_x() - p[2].get_x());
  ...
  final.push_back(Area); final.push_back (omega_y);
  final.push_back(omega_x); final.push_back(angle);
...
int main()
{
  ...
  try {
    ...
    vpHomogeneousMatrix cMo(0.15, -0.1, 1., vpMath::rad(-20), vpMath::rad(30), vpMath::rad(20)); // -20, 30
